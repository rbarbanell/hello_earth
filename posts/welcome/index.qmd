...
index_goat %>%
  select(num_year,age,gametype,team,overallbpm,year)%>%
  filter( gametype == "Regular Season")%>%
  select(-gametype)%>%
  kbl(booktabs = TRUE, caption = "Regular Season for Players", full_width = F)%>%
  kable_styling(latex_options = "striped")%>%
  pack_rows("Michael Jordan", 1, 14) %>%
  pack_rows("Kobe Bryant", 16, 36)%>%
  pack_rows("Lebron James", 36,51)
...
index_goat %>%
  ...
  select(-gametype)%>%
  kbl(booktabs = TRUE, caption = "Playoffs for Players", full_width = F)%>%
  kable_styling(latex_options = "scale_down")%>%
  pack_rows("Michael Jordan", 1, 13) %>%
  pack_rows("Kobe Bryant", 14, 28)%>%
  pack_rows("Lebron James", 29,41)
```
